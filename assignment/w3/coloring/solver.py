...
def mip_solver(node_count, edges, max_degree_vertex):
    solver = pywraplp.Solver("coloring", pywraplp.Solver.CBC_MIXED_INTEGER_PROGRAMMING)
    color_map = []
    for i in range(node_count):
        color_map.append([])
        for j in range(node_count):
            color_map[i].append(solver.IntVar(0,1,str(i) + str(j)))
    
    for vertex_color in color_map:
        solver.Add(sum(vertex_color) == 1)
    for edge in edges:
        for i in range(node_count):
            solver.Add((color_map[edge[0]][i] + color_map[edge[1]][i]) <= 1)
    obj = solver.IntVar(0, node_count, 'obj')
    vertex = []
    for v in range(node_count):
        vertex.append(solver.IntVar(v,v,str(v)))
    for c in range(node_count):
        for v in range(node_count):
            solver.Add(obj >= vertex[v])
    solver.Minimize(obj)
    print(solver.NumConstraints())

def GraphColoringProgram(node_count, edges, max_degree_vertex):
    # creates the model

    model = cp_model.CpModel()
    
    color = [[model.NewIntVar(0, 1, str(i)) for i in range(node_count)] for _ in range(node_count)]
    model.Add(color[max_degree_vertex] == 0)

    # for c in color:
    #     model.Add(c <= max_clique)

    for e in edges:
        model.Add(color[e[0]] != color[e[1]])

    solver = cp_model.CpSolver()
    status = solver.Solve(model)

    solution = [i for i in range(node_count)]
    color_count = node_count
    if status == cp_model.FEASIBLE:
        for i in range(node_count):
            solution[i] = solver.Value(color[i])
    
        color_count = max(solution) + 1
    
    return color_count, solution

def find_3_cliques(edge_count, edges):
    num_run = int(0.1 * edge_count)
    cliques = []
    for i in range(num_run):
        edge_1 = edges[random.randint(0, edge_count-1)]
        edge_2 = edges[random.randint(0, edge_count-1)]
        edge_3 = edges[random.randint(0, edge_count-1)]
        c = set(edge_1 + edge_2 + edge_3)
        if len(c) == 3:
            cliques.append(c)
    return cliques

def constraint_programing(edges, edge_count, node_count, max_degree):
    model = cp_model.CpModel()
    cvar = [model.NewIntVar(0,int(math.sqrt(2*edge_count)),str(i)) for i in range(node_count)]
    cliques = find_3_cliques(edge_count, edges)
    solution =[]
    ...
    for clique in cliques:
        model.Add(cvar[clique[0]] != cvar[clique[1]])
        model.Add(cvar[clique[0]] != cvar[clique[2]])
        model.Add(cvar[clique[1]] != cvar[clique[2]])
    ...
    while(1):
        end_time = time.time()
        if(int(end_time)-int(start_time) > 60*minute):
            break
        model.Add(cvar[max_degree] == 0)
        for i in range(node_count):
            model.Add(cvar[i] < count_color)
        solver = cp_model.CpSolver()
        solver.parameters.max_time_in_seconds = 60*minute
        status = solver.Solve(model)
        if( status == cp_model.FEASIBLE):
            solution.clear()
            for i in range(node_count):
...
def solve_it(input_data):
    ...
    order_degree = sorted(range(node_count), key=lambda k: degree[k])
    max_degree = order_degree[-1]

    # clique_cand = []
    # count_degree = [0]*(degree[max_degree] + 1)
    # for i in range(degree[max_degree] + 1):
    #     count_degree[i] = degree.count(i)
    #     indexs = [i for i, e in enumerate(count_degree) if e == count_degree[i]]
    #     clique_cand.append(min(indexs[-1], count_degree[i]))
    
    # max_clique = max(clique_cand)
    # print(max_clique)
    # print(max_clique)
    # print(count_degree)
    ...
    # color_count, solution = GraphColoringProgram(node_count, edges, max_degree)
    # mip_solver(node_count, edges, max_degree)
    count_color, solution = constraint_programing(edges, edge_count, node_count, max_degree)
...
#             #In case of no assignment is found (infeasible solution), stopr
#             if(color == -1):
#                 break
...
# #Assign least frequent color to a node
...
#         AssingColor(graph,assignment[0],assignment[1],False)
...
#     #beta = int(1.25*math.sqrt(graph.length)) 
